...
class ConversationSimulatorLazy:
    def __init__(self, sample, assistant_model="gpt-4o-mini", system_model="gpt-4o-mini", user_model="gpt-4o-mini", assistant_temperature=1.0, user_temperature=1.0, dataset_fn=None, log_folder=None): # max_turns=10
    ...
    def run(self, verbose=False, save_log=True):
        ...
        while not is_completed:
            ...
            user_response, hint_revealed_id, cost_usd = self.user_agent.generate_response(self.trace, self.sample, temperature=self.user_temperature)
            ...
            assistant_response_obj = generate(extract_conversation(self.trace, to_str=False), model=self.assistant_model, temperature=self.assistant_temperature, step="lazy-simulator-assistant-generation", return_metadata=True, max_tokens=max_assistant_tokens)
        ...
        if save_log:
            ...
            if self.run_with_custom_temperature:
                conv_type = f"lazy-at{self.assistant_temperature}-ut{self.user_temperature}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, default="database", choices=["database", "translation", "summary", "math", "apis", "data2text", "python"])
    ...
    conversation_simulator = ConversationSimulatorLazy(task_name=args.task, sample=sample, assistant_model=args.assistant_model, system_model=args.system_model, user_model=args.user_model) # , max_turns=args.max_turns
