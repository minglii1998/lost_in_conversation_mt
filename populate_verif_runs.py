...
def single_run(task_name, todo, version):
    try:
        if todo["conv_type"] == "full":
            conversation_simulator = ConversationSimulatorFull(task_name, todo["sample"], version=version, assistant_model=todo["assistant_model"], system_model=todo["system_model"])
            return conversation_simulator.run(verbose=args.verbose, save_log=True)
        elif todo["conv_type"] == "concat":
            conversation_simulator = ConversationSimulatorFull(task_name, todo["sample"], version=version, assistant_model=todo["assistant_model"], system_model=todo["system_model"], run_concat=True)
            return conversation_simulator.run(verbose=args.verbose, save_log=True)
        elif todo["conv_type"] == "shuffle-concat":
            conversation_simulator = ConversationSimulatorFull(task_name, todo["sample"], version=version, assistant_model=todo["assistant_model"], system_model=todo["system_model"], run_shuffle_concat=True)
            return conversation_simulator.run(verbose=args.verbose, save_log=True)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_fn", type=str, default=None, help="Dataset file to use")

    parser.add_argument("--N_full_runs", type=int, default=3, help="Number of full runs per model")
    parser.add_argument("--N_concat_runs", type=int, default=0, help="Number of concat runs per model")
    parser.add_argument("--N_shuffle_concat_runs", type=int, default=3, help="Number of shuffle-concat runs per model")
    parser.add_argument("--verif_models", type=str, nargs="+", default=["t-gpt-4o", "claude3.5-sonnet", "gemini-1.5-pro"], help="Assistant models to use")
    ...
    conv_types = ["full", "concat", "shuffle-concat"]
    N_runs = {
    ...
    fn = args.dataset_fn.split("/")[-1].replace(".json", "")
    task_name = fn.split("_")[1]
    with open(args.dataset_fn, "r") as f:
        samples = json.load(f)

    # Format: data/lazy_humaneval_{version}.json
    version = fn.split("_")[-1]
    ...
    for (sample_id, todo_tasks) in todo_samples.items():
        ...
        for (model, conv_type) in todo_tasks:
            all_todos += [{"sample": sample, "assistant_model": model, "conv_type": conv_type, "system_model": args.system_model}] * todo_tasks[(model, conv_type)]

        with ThreadPoolExecutor(max_workers=args.N_workers) as executor:
            results = list(executor.map(single_run, [task_name]*len(all_todos), all_todos, [version]*len(all_todos)))
